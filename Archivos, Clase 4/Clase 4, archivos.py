...
def create_raw_map(filename, rows, col, char):
    with open (filename, 'w') as file: #abre el archivo
        line = char * col  #crea el string con los caracteres de una linea
...
def read_map(filename):
    ...
    with open (filename, 'r') as file: #abre el archivo
        for line in file:   #el file es iterable, se recorre por linea
            matriz.append(line.rstrip('\n')) #hace push en la matriz y se guarda como lista, el [:-1] quita el \n
    return matriz
...
#no se esta marcando correctamente
def mark_path(filename, path, char='*'):
    for r, c in path:
        change_character(filename, r, c, char)


#si el tesoro esta encerrado de @ no lo esta detectando como no encontrado
