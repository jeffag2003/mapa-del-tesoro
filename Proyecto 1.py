...
create_raw_map("mapa.txt", 10, 10, '.')
print("Mapa creado")

obstaculos = [(6,6), (6,7), (6,8), (7,6), (7,8), (8,6), (8,7), (8,8)]
for r,c in obstaculos:
    change_character("mapa.txt", r, c, '#')
change_character("mapa.txt", 7, 7, 'T')
print("Obstáculos y tesoro asignados")

print("Contenido del mapa:")
for line in read_map("mapa.txt"):
    print(line)

start = (0, 0)
camino = find_treasure("mapa.txt", start)
print("Camino:", camino)

if camino:
    mark_path("mapa.txt", camino, '*')

map_GUI("mapa.txt", start, camino if camino else [], bool(camino))
